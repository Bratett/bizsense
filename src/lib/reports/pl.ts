...
export async function getProfitAndLoss(
  ...
  const grossMarginPct =
    revenueTotal === 0
      ? 0
      : Math.round((grossProfit / revenueTotal) * 10_000) / 100  // e.g. 42.5
  ...
  if (includePrior) {
    ...
    result.revenue.lines  = revenueLines.map(l => ({ ...l, priorNetBalance: priorRevMap.get(l.accountId) ?? 0 }))
    result.cogs.lines     = cogsLines.map(l => ({ ...l, priorNetBalance: priorCogMap.get(l.accountId) ?? 0 }))
    result.expenses.lines = expenseLines.map(l => ({ ...l, priorNetBalance: priorExpMap.get(l.accountId) ?? 0 }))
