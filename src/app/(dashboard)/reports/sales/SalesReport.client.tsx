  ...
  return (
    <thead>
      <tr className="border-b border-gray-200 bg-gray-50">
        ...
        <th className="py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 pr-4">
          Orders
        </th>
...
function TableRow({
  ...
  const href = isProduct
    ? `/inventory/${line.entityId}`
    : isCustomer
      ? `/customers/${line.entityId}`
      : null
  ...
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className="py-2.5 pr-4 text-right text-sm tabular-nums text-gray-900">
        {formatGhs(line.revenue)}
      </td>
      <td className="py-2.5 pr-4 text-right text-sm tabular-nums text-gray-600">
        {formatGhs(line.cogsTotal)}
      </td>
      <td
        className={`py-2.5 pr-4 text-right text-sm tabular-nums ${line.grossProfit < 0 ? 'text-red-600' : 'text-gray-900'}`}
      >
